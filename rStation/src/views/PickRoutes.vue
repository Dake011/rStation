        ...
        <div class="row menu-items">
            ...
            <div class="menu-data row col-sm-9">
                <div class="station-form">
                    ...
                    <form class="form" id="registrationForm" v-if="showInputs == 1">
                        ...
                        <div class="row" v-if="showInputs == 1">
                            <div class="form-group col-4">
                                <div class="col-sm-12">
                    ...
                    <div class="alert-box" v-if="showInputs == 2"> 
                        <span>Another direction.</span>
                    </div>
                    <form class="form" id="registrationForm" v-if="false">
                        ...
                        <div class="row" v-if="showInputs == 2">
                            <div class="form-group col-4">
                                <div class="col-sm-12">
                                    <label for="stationName"><h4>Choose a Train</h4></label>
                                    <select class="browser-default custom-select">
                ...
                <input type="button" class="btn btn-danger" @click="showRoute()" value="Show Data">
                <input type="button" class="btn btn-danger" @click="saveRoute()" value="Save Route">
                <input type="button" class="btn btn-danger" @click="notifyPassengers()" value="Notify Passengers">
...
import json from '../assets/kz.json'
    ...
    export default {
        ...
        methods: {
            ...
            showRoute(){
                let selectedRoute = this.clickedCoords;
                let selectedRouteOpposite = this.clickedCoordsOpposite;
                let routes =  JSON.parse(localStorage.getItem('allRoutes'))
                var objectsAreSame = true;
                var objectsAreSameOpposite = true;
                routes.forEach(route => {
                    if(route.length == selectedRoute.length){
                        for(var x in route) {
                            if(route[x].city !== selectedRoute[x].city) {
                                objectsAreSame = false;
                            }
                            route.reverse()
                            if(route[x].city !== selectedRouteOpposite[x].city) {
                                objectsAreSameOpposite = false;
                            }
                            route.reverse()
                        }
                        if(objectsAreSame) {
                            this.showInputs++;
                            this.clickedCoords = route;
                        }
                        if(objectsAreSameOpposite) {
                            // this.showInputs++;
                            this.clickedCoordsOpposite = route;
                        }
                    }
                })
            },
